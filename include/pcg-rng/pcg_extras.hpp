...
namespace pcg_extras {
    ...
    operator<<(std::basic_ostream<CharT, Traits> &out, pcg128_t value) {
        ...
        if (want_hex) {
            ...
            out << lowpart;
            out.setf(oldflags);
            if (highpart != 0) { out.fill(oldfill); }
            return out;
        }
    ...
    inline itype unxorshift(itype x, bitcount_t bits, bitcount_t shift) {
        if (2 * shift >= bits) { return x ^ (x >> shift); }
...
#pragma intrinsic(_rotr, _rotr64, _rotr8, _rotr16)
    ...
    SrcIter uneven_copy_impl(
       SrcIter src_first, DestIter dest_first, DestIter dest_last, std::true_type
    ) {
    ...
    template <class SrcIter, class DestIter>
    SrcIter uneven_copy_impl(
       SrcIter src_first, DestIter dest_first, DestIter dest_last, std::false_type
    ) {
        typedef typename std::iterator_traits<SrcIter>::value_type src_t;
        typedef typename std::iterator_traits<DestIter>::value_type dest_t;
