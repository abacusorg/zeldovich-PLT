...
class SplineFunction {
  ...
  public:
    SplineFunction() { n = nmax = 0; x = y = y2 = NULL; }
    SplineFunction(int nn) { n = nmax = 0; x = y = y2 = NULL; allocate(nn); }
    ~SplineFunction() { deallocate(); }
    void deallocate() {
        delete []x; delete []y; delete []y2; n = nmax = 0;
    }
    int allocate(int nn) {
	if (nmax!=0) deallocate();
        nmax = nn;
	x = new double[nmax]; y = new double[nmax]; y2 = new double[nmax];
	assert(x!=NULL); assert(y!=NULL); assert(y2!=NULL);
	return 0;   
    }
    int reallocate(int nn) {
	// Check if we got lucky first
        if (nn==nmax) return 0;
	    else return allocate(nn);
    }
    int size() { return n; };
