...
ParseHeader::~ParseHeader(void) { delete phdriver; }
...
void ParseHeader::installscalar(
   std::string name, long long int &var, bool must_define
) {
...
template <typename T>
void ParseHeader::installvector(
   std::string name, T *var, int len, int stride, bool must_define
) {
...
void ParseHeader::installvector(
   std::string name, int *var, int len, int stride, bool must_define
) {
    phdriver->InstallSym(name.c_str(), var, INTEGER, len, stride, must_define);
}

void ParseHeader::installvector(
   std::string name, float *var, int len, int stride, bool must_define
) {
    phdriver->InstallSym(name.c_str(), var, FLOAT, len, stride, must_define);
}

void ParseHeader::installvector(
   std::string name, double *var, int len, int stride, bool must_define
) {
...
template <>
void ParseHeader::installvector(
   std::string name, char **var, int len, int stride, bool must_define
) {
...
void ParseHeader::ReadHeader(HeaderStream &in) {
    in.ReadHeader();
    assert(
       in.buffer[in.bufferlength - 1] == 0x0 && in.buffer[in.bufferlength - 2] == 0x0
    );
    ...
    if (result != 0) {
        std::cerr << "HS::parseit: there were errors parsing \"" << in.name
                  << "\" ...exiting.\n";
        exit(1);
    }
